...
\If {$\sum_{w(e(i,u))} = w(u)$}
\State {$S_u \gets true$} \Comment $u$ joins the cover \label{alglin:dgmm-join-cover-R}
\State {state $\gets \cD$}
\Else 
\State {state $\gets \cC$}
\EndIf
...
\If {$\sum_{w(e(i,u))} = w(u)$}
\State $S_u \leftarrow true$ \Comment $u$ joins the cover\label{alglin:dgmm-join-cover-W}
